# AI Development Guide for flows-auth

This document contains detailed guidance for AI assistants (Claude Code, etc.) working on flows-auth. This content was moved from CLAUDE.md to reduce token usage while preserving critical knowledge.
...
### 3. Client-Side vs Development Infrastructure
...
- **Demo applications** - Should demonstrate client-side patterns but may run on SSR frameworks for convenience

**Key principle**: Anything that ships to customers or gets imported by customers must work in pure client-side environments. Development and demo infrastructure can use whatever is most efficient.

### 4. Library Rebuilding
...
**📋 FORMAL POLICY**: See [API Contract Testing Policy](../testing/API_CONTRACT_TESTING_POLICY.md)
...
- **`flows-client`**: Database schema and functionality with admin demo application
...
- **`CLAUDE.md`** → AI development patterns, common mistakes
