...
import { z } from 'zod';
import { UserMetadataSchema } from './metadata-schema.js';
...
export const AuthCoreStateSchema = z.object({
  ...
  expiresAt: z.string().nullable(), // ISO 8601 timestamp string
  refreshedAt: z.string().nullable(), // ISO 8601 timestamp string - when token was last refreshed (spam protection)
  supabase_token: z.string().nullable(), // Supabase JWT for database access with RLS
  supabase_expires_at: z.string().nullable(), // ISO 8601 timestamp string - Supabase token expiration
