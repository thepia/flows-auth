...
/**
 * Common auth events with type-safe data
 */
...
export function createTypedEventEmitters(eventStore: ReturnType<typeof createEventStore>) {
  ...
  return {
    signInStarted: (data: AuthEvents['sign_in_started']) => 
      emit('sign_in_started', data),
    
    signInSuccess: (data: AuthEvents['sign_in_success']) => 
      emit('sign_in_success', data),
    
    signInError: (data: AuthEvents['sign_in_error']) => 
      emit('sign_in_error', data),
    
    signOut: (data: AuthEvents['sign_out'] = {}) => 
      emit('sign_out', data),
    
    tokenRefreshed: (data: AuthEvents['token_refreshed']) => 
      emit('token_refreshed', data),
    
    sessionExpired: (data: AuthEvents['session_expired']) => 
      emit('session_expired', data),
    
    passkeyUsed: (data: AuthEvents['passkey_used']) => 
      emit('passkey_used', data),
    ...
    registrationError: (data: AuthEvents['registration_error']) => 
      emit('registration_error', data),
  };
}
...
export function createEventMiddleware(
  ...
  return {
    ...
    bridge: <T>(
      ...
      toCallback: (data: AuthEventData) => T
    ) => {
    ...
    transform: (
      ...
      return eventStore.getState().on(fromEvent, (data) => {
        try {
          ...
          eventStore.getState().emit(toEvent, transformedData);
        } catch (error) {
