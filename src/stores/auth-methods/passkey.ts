...
export function createPasskeyStore(options: StoreOptions) {
  const { config, devtools: enableDevtools = false, name = 'passkey', api } = options;

  const storeImpl = (
    set: (
      partial: Partial<PasskeyStore> | ((state: PasskeyStore) => Partial<PasskeyStore>)
    ) => void,
    get: () => PasskeyStore
  ) => ({
    ...initialState,
 ...
 */
export function getPasskeyReadiness(passkeyStore: ReturnType<typeof createPasskeyStore>) {
