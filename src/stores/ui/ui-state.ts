...
export function createUIStore(options: StoreOptions) {
  const { config, devtools: enableDevtools = false, name = 'ui' } = options;
    ...
    setEmail: (email: string) => {
      ...
      if (emailChanged) {
        ...
        set({
          ...
          signInState:
            currentState.signInState === 'userChecked' ? 'emailEntry' : currentState.signInState
        });
      } else {
        set({ email });
      }
