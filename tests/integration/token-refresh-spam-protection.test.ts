...
describe('Token Refresh Spam Protection', () => {
  ...
  describe('refreshedAt Field Persistence', () => {
    it('should set refreshedAt when tokens are updated', async () => {
      ...
      expect(refreshedAtMs).toBeGreaterThanOrEqual(beforeUpdate);
    ...
    it('should update refreshedAt on subsequent token refreshes', async () => {
      ...
      // refreshedAt is now an ISO string, parse both to compare
      const secondRefreshTimeMs = new Date(secondRefreshTime as string).getTime();
      const firstRefreshTimeMs = new Date(firstRefreshTime as string).getTime();
      expect(secondRefreshTimeMs).toBeGreaterThan(firstRefreshTimeMs);
  ...
  describe('Minimum Refresh Interval Enforcement', () => {
    it('should enforce 60-second minimum between refreshes', async () => {
      ...
      // refreshedAt is now an ISO string, parse it to compare
      const refreshedAtMs = new Date(authStore.core.getState().refreshedAt as string).getTime();
      const timeSinceRefresh = Date.now() - refreshedAtMs;
    ...
    it('should allow refresh after 60-second minimum has passed', async () => {
      ...
      const secondRefreshTimeMs = new Date(secondRefreshTime as string).getTime();
      const firstRefreshTimeMs = new Date(firstRefreshTime as string).getTime();
      const timeDiff = secondRefreshTimeMs - firstRefreshTimeMs;
  ...
  describe('Refresh Scheduling Logic', () => {
    it('should schedule refresh based on expires_in from server', async () => {
      ...
      const expiresAtMs = new Date(state.expiresAt as string).getTime();
    ...
    it('should respect refreshBefore config when scheduling', async () => {
      ...
      const expectedRefreshTime = Math.max(timeUntilExpiry - refreshBefore, 60 * 1000);
