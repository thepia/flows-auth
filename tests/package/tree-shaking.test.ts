...
describe('Tree-Shaking Support (Built Package)', () => {
  ...
  it('should support modular imports via package.json exports', async () => {
    ...
    const storeExports = {
      './stores': '../../src/stores-new/index.ts',
      './stores/core': '../../src/stores-new/core/index.ts', 
      './stores/auth-methods': '../../src/stores-new/auth-methods/index.ts',
      './stores/ui': '../../src/stores-new/ui/index.ts',
      './stores/adapters': '../../src/stores-new/adapters/index.ts'
    };
    ...
    for (const [exportPath, actualPath] of Object.entries(storeExports)) {
      try {
        ...
        const createFunctions = exportNames.filter(name => 
          typeof module[name] === 'function' && name.startsWith('create')
        );
        
        expect(createFunctions.length).toBeGreaterThan(0);
        console.log(`✅ Export ${exportPath} has ${createFunctions.length} create functions:`, createFunctions);
      } catch (error) {
  ...
  it('should demonstrate bundle size benefits through selective imports', async () => {
    ...
    expect(fullStore.api.signInWithEmail).toBeDefined(); // Includes email auth too
    
    // Selective import (would be smaller in real bundling scenario)
    const { createPasskeyStore } = await import('../../src/stores-new/auth-methods/index.ts');
    const { createAuthCoreStore } = await import('../../src/stores-new/core/index.ts');
  ...
  it('should support framework-specific imports without cross-contamination', async () => {
    ...
    const { createSvelteAdapter } = await import('../../src/stores-new/adapters/index.ts');
    const { createAuthCoreStore } = await import('../../src/stores-new/core/index.ts');
  ...
  it('should verify dist files structure matches package.json exports', async () => {
    ...
    const expectedPaths = [
      'dist/index.js',
      'src/stores-new/index.ts',
      'src/stores-new/core/index.ts',
      'src/stores-new/auth-methods/index.ts', 
      'src/stores-new/ui/index.ts',
      'src/stores-new/adapters/index.ts'
    ];
