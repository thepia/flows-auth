...
describe('Flow Data Generation Logic', () => {
  describe('SessionStateMachineFlow Data Generation', () => {
    ...
    function createSessionFlowData(authState: string) {
      const flowNodes = Object.entries(authStateCategories).flatMap(([categoryName, config]) =>
          ...
          return {
            ...
            style: {
              ...
              boxShadow: isCurrentState
                ? '0 0 0 2px rgba(251, 191, 36, 0.3), 0 4px 8px rgba(0, 0, 0, 0.15)'
  ...
  describe('Error Prevention Logic', () => {
    it('should prevent NaN coordinates with Math.max', () => {
      const testValues = [100, 0, -50, Number.NaN, undefined, null];
